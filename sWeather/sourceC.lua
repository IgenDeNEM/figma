  ...
  for i = n, 1, -1 do
    if p >= ambientColor[i][1] then
      if i >= n then
        ...
        return ambientColor[i][2] + (ambientColor[i + 1][2] - ambientColor[i][2]) * prog, ambientColor[i][3] + (ambientColor[i + 1][3] - ambientColor[i][3]) * prog, ambientColor[i][4] + (ambientColor[i + 1][4] - ambientColor[i][4]) * prog
      end
    end
  end
  ...
  for i = n, 1, -1 do
    if p >= spriteShadow[i][1] then
      if i >= n then
        ...
        return spriteShadow[i][2] + (spriteShadow[i + 1][2] - spriteShadow[i][2]) * prog, spriteShadow[i][3] + (spriteShadow[i + 1][3] - spriteShadow[i][3]) * prog, spriteShadow[i][4] + (spriteShadow[i + 1][4] - spriteShadow[i][4]) * prog
      end
    end
  end
  ...
  for i = n, 1, -1 do
    if p >= ambientObjColor[i][1] then
      if i >= n then
        ...
        return ambientObjColor[i][2] + (ambientObjColor[i + 1][2] - ambientObjColor[i][2]) * prog, ambientObjColor[i][3] + (ambientObjColor[i + 1][3] - ambientObjColor[i][3]) * prog, ambientObjColor[i][4] + (ambientObjColor[i + 1][4] - ambientObjColor[i][4]) * prog
      end
    end
  end
...
function processSky(p)
  ...
  local r, g, b = processAmbientColor(p)
  setWorldProperty("AmbientColor", r, g, b)
  setWorldProperty("DirectionalColor", r, g, b)
  local r, g, b = processAmbientObjColor(p)
  setWorldProperty("AmbientObjColor", r, g, b)
  local a, b, c = processSpriteShadow(p)
  setWorldProperty("SpriteSize", a)
  setWorldProperty("SpriteBrightness", b)
  setWorldProperty("ShadowStrength", c)
end
local forceH, forceM, forceS, forceW = false, false, false, false
addEvent("gotTimeChange", true)
addEventHandler("gotTimeChange", getRootElement(), function(h, m, s, w)
  timeH, timeM, timeS = h, m, s, w
  if w then
    forceW = w
  end
  if not forceH then
...
addEvent("setForceTime", true)
addEventHandler("setForceTime", root, setForceTime)
