...
setTimer(function()
	if getElementData(localPlayer, "loggedIn") then
		hasParachute = exports.sItems:playerHasItem(107)
	end
end, 5000, 0)
...
local function onRender()
	...
	if tickDiff > 0 then
		if hasParachute and not getElementData(localPlayer, "parachuting") and not isPedWearingJetpack(localPlayer) and not isElementAttached(localPlayer) and not isPedDead(localPlayer) then
...
function warningText()
	warning = true
	dxDrawText ( "Nyisd ki az ejtőernyőd!",
