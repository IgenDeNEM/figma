...
function dxText:create(text, x, y, relative)
  ...
  if not (type(text) == "string" and tonumber(x)) or not tonumber(y) then
    return false
  end
  local new = {}
...
function dxText:type(type, ...)
  if not validTypes[type] then
    return self.strType, unpack(self.tAttributes)
  end
  self.strType = type
  self.tAttributes = {
    ...
...
function dxText:align(horzA, vertA)
  if not validAlignTypes[horzA] then
    return self.bHorizontalAlign, self.bVerticalAlign
  end
