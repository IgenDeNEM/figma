complex = {}
local complex_meta = {}
local _retone = function()
  return 1
end
local _retminusone = function()
  return -1
end
function complex.to(num)
  ...
  if type(num) == "string" then
    ...
    if real then
      if string.lower(string.sub(real, 1, 1)) == "e" or string.lower(string.sub(imag, 1, 1)) == "e" then
        return
      end
      if imag == "" then
        if sign == "+" then
...
function complex.mul(cx1, cx2)
  return setmetatable({
    cx1[1] * cx2[1] - cx1[2] * cx2[2],
    cx1[1] * cx2[2] + cx1[2] * cx2[1]
  }, complex_meta)
end
...
function complex.div(cx1, cx2)
  ...
  return setmetatable({
    (cx1[1] * cx2[1] + cx1[2] * cx2[2]) / val,
    (cx1[2] * cx2[1] - cx1[1] * cx2[2]) / val
  }, complex_meta)
end
...
function complex.sqrt(cx)
  ...
  return setmetatable({
    math.sqrt((cx[1] + len) / 2),
    sign * math.sqrt((len - cx[1]) / 2)
  }, complex_meta)
end
...
function complex.exp(cx)
  ...
  return setmetatable({
    expreal * math.cos(cx[2]),
    expreal * math.sin(cx[2])
  }, complex_meta)
end
...
function complex_meta.__add(cx1, cx2)
  local cx1, cx2 = complex.to(cx1), complex.to(cx2)
  return complex.add(cx1, cx2)
end
function complex_meta.__sub(cx1, cx2)
  local cx1, cx2 = complex.to(cx1), complex.to(cx2)
  return complex.sub(cx1, cx2)
end
function complex_meta.__mul(cx1, cx2)
  local cx1, cx2 = complex.to(cx1), complex.to(cx2)
  return complex.mul(cx1, cx2)
end
function complex_meta.__div(cx1, cx2)
  local cx1, cx2 = complex.to(cx1), complex.to(cx2)
  return complex.div(cx1, cx2)
end
function complex_meta.__pow(cx, num)
  if num == "*" then
    return complex.conjugate(cx)
  end
  return complex.pow(cx, num)
end
function complex_meta.__unm(cx)
  return setmetatable({
    -cx[1],
    -cx[2]
  }, complex_meta)
end
function complex_meta.__eq(cx1, cx2)
  if cx1[1] == cx2[1] and cx1[2] == cx2[2] then
    return true
  end
  return false
end
function complex_meta.__tostring(cx)
  return tostring(complex.tostring(cx))
end
function complex_meta.__concat(cx, cx2)
  return tostring(cx) .. tostring(cx2)
end
function complex_meta.__call(...)
end
