...
processsightlangStaticImage[2] = function()
  if not isElement(sightlangStaticImage[2]) then
    ...
    sightlangStaticImage[2] = dxCreateTexture("files/shadow_ear.dds", "argb", true)
  end
  if not sightlangStatImgHand then
    sightlangStatImgHand = true
    addEventHandler("onClientPreRender", getRootElement(), sightlangStatImgPre, true, "high+999999999")
...
processsightlangStaticImage[4] = function()
  if not isElement(sightlangStaticImage[4]) then
    ...
    sightlangStaticImage[4] = dxCreateTexture("files/ear.dds", "argb", true)
  end
  if not sightlangStatImgHand then
    sightlangStatImgHand = true
    addEventHandler("onClientPreRender", getRootElement(), sightlangStatImgPre, true, "high+999999999")
...
processsightlangStaticImage[5] = function()
  if not isElement(sightlangStaticImage[5]) then
    ...
    sightlangStaticImage[5] = dxCreateTexture("files/eyebg.dds", "argb", true)
  end
  if not sightlangStatImgHand then
    sightlangStatImgHand = true
    addEventHandler("onClientPreRender", getRootElement(), sightlangStatImgPre, true, "high+999999999")
...
processsightlangStaticImage[6] = function()
  if not isElement(sightlangStaticImage[6]) then
    ...
    sightlangStaticImage[6] = dxCreateTexture("files/eye.dds", "argb", true)
  end
  if not sightlangStatImgHand then
    sightlangStatImgHand = true
    addEventHandler("onClientPreRender", getRootElement(), sightlangStatImgPre, true, "high+999999999")
...
processsightlangStaticImage[7] = function()
  if not isElement(sightlangStaticImage[7]) then
    ...
    sightlangStaticImage[7] = dxCreateTexture("files/eyeshut.dds", "argb", true)
  end
  if not sightlangStatImgHand then
    sightlangStatImgHand = true
    addEventHandler("onClientPreRender", getRootElement(), sightlangStatImgPre, true, "high+999999999")
...
processsightlangStaticImage[8] = function()
  if not isElement(sightlangStaticImage[8]) then
    ...
    sightlangStaticImage[8] = dxCreateTexture("files/head.dds", "argb", true)
  end
  if not sightlangStatImgHand then
    sightlangStatImgHand = true
    addEventHandler("onClientPreRender", getRootElement(), sightlangStatImgPre, true, "high+999999999")
...
function loadedItems()
  for item in pairs(bunnyData) do
    bunnyNames[item] = sightexports.sItems:getItemName(item)
  end
end
...
local sleepTime = 2500
local bunnySleeping = 0
local fadeInSpeed = 3
...
function tryToStartBunnyOpening(dbID)
  if not minigameStarted then
    triggerServerEvent("tryToStartBunnyOpening", localPlayer, dbID)
  end
end
addEvent("startBunnyOpening", true)
addEventHandler("startBunnyOpening", getRootElement(), function(itemId, rnd1, rnd2, rnd3, rnd4, rnd5)
  if not minigameStarted then
    addEventHandler("onClientPreRender", getRootElement(), preRenderBunnyOpen)
    addEventHandler("onClientRender", getRootElement(), renderBunnyOpen)
    minigameStarted = true
  end
  snow = {}
  words = {}
  lastWord = 0
  lastSleep = getTickCount()
  lastSnow = 0
  itemList = {}
  itemEye = false
  targetAlpha = 0
  ...
  winItem = itemId
  winItemId = false
  bunnySleeping = sleepTime * 3
  fadeInSpeed = 3
  overallAlpha = 0
  local d = 384
  overallTime = 1 / fadeInSpeed * 1000 + bunnySleeping + spinStop + (targetSpinSpeed - spinMin) / spinDeceleration * 1000 + (d / spinMin * 7 + d / spinMin2) / 8 * 1000
  rotateEnded = false
  musicStart = false
...
function preRenderBunnyOpen(delta)
  local now = getTickCount()
  local y = 472
  for i = #words, 1, -1 do
  ...
  if lastSnow < 0 then
    lastSnow = math.random(5000, 6000) / 10
  ...
  if overallAlpha < 1 and not rotateEnded then
    ...
    if bunnySleeping <= 500 then
      targetAlpha = math.min(1, 1 - bunnySleeping / 500)
    end
    if bunnySleeping <= 1750 and not musicStart then
      musicStart = 750
    ...
    if 0 < bunnySleeping then
      bunnySleeping = bunnySleeping - delta
    else
      local md = 384
      local minX = md
  ...
  if rotateEnded then
    ...
    if 10000 < d then
      ...
      if overallAlpha < 0 then
        ...
        snow = {}
        words = {}
        removeEventHandler("onClientPreRender", getRootElement(), preRenderBunnyOpen)
        removeEventHandler("onClientRender", getRootElement(), renderBunnyOpen)
...
function renderBunnyOpen()
  ...
  local p3 = now % 1600 / 800
  if 1 < p3 then
    p3 = 2 - p3
  end
  p3 = getEasingValue(p3, "InOutQuad")
  ...
  for i = 1, #snow do
    if not snow[i][8] then
      ...
      dxDrawImage(x, y, s, s, dynamicImage("files/" .. snow[i][1] .. ".dds"), snow[i][5] + 15 - sin * 30, 0, 0, tocolor(255, 255, 255, a * overallAlpha))
    end
  end
  ...
  if 0 < bunnySleeping then
    local sleepDelta = now - lastSleep
    if sleepDelta > sleepTime then
      local ex = ix + hx + 290 - 22 * hc + 12 * hs
      local ey = iy + hy + 156 - 22 * hs - 12 * hc
      spawnWord("zzz", ex, ey)
      if 2500 < bunnySleeping then
        playSound("files/snore.mp3")
      end
      lastSleep = now
    end
  ...
  dxDrawImage(ix + 10 * p2 - 5, iy + 5, 500 - 20 * p2, 372, sightlangStaticImage[0], 0, 0, 0, tocolor(255, 255, 255, 150 * overallAlpha))
  sightlangStaticImageUsed[1] = true
  if sightlangStaticImageToc[1] then
    processsightlangStaticImage[1]()
  end
  dxDrawImage(ix + hx - 5, iy + hy + 5, 500, 372, sightlangStaticImage[1], hr, 10, 0, tocolor(255, 255, 255, 150 * overallAlpha))
  local ex, ey = ix + hx + 250 + 14, iy + hy + 186
  local exe = ex - hc * 67 + hs * 117
  local eye = ey - hs * 67 - hc * 117
  local exe2 = ex + hc * 39 + hs * 106
  local eye2 = ey + hs * 39 - hc * 106
  sightlangStaticImageUsed[2] = true
  if sightlangStaticImageToc[2] then
    processsightlangStaticImage[2]()
  end
  dxDrawImage(exe - 128 - 5, eye - 128 + 5, 256, 256, sightlangStaticImage[2], hr + 14.3 + 14 * p3, 0, 0, tocolor(255, 255, 255, 150 * overallAlpha))
  sightlangStaticImageUsed[2] = true
  if sightlangStaticImageToc[2] then
    processsightlangStaticImage[2]()
  end
  dxDrawImage(exe2 + 128 - 5, eye2 - 128 + 5, -256, 256, sightlangStaticImage[2], hr - 20 - 14 * p3, 0, 0, tocolor(255, 255, 255, 150 * overallAlpha))
  ...
  dxDrawImage(exe - 128, eye - 128, 256, 256, sightlangStaticImage[4], hr + 14.3 + 14 * p3, 0, 0, tocolor(255, 255, 255, 255 * overallAlpha))
  sightlangStaticImageUsed[4] = true
  if sightlangStaticImageToc[4] then
    processsightlangStaticImage[4]()
  end
  dxDrawImage(exe2 + 128, eye2 - 128, -256, 256, sightlangStaticImage[4], hr - 20 - 14 * p3, 0, 0, tocolor(255, 255, 255, 255 * overallAlpha))
  if bunnySleeping <= 0 then
    sightlangStaticImageUsed[5] = true
    if sightlangStaticImageToc[5] then
      processsightlangStaticImage[5]()
    end
    dxDrawImage(ix + hx, iy + hy, 500, 372, sightlangStaticImage[5], hr, 14, 0, tocolor(255, 255, 255, 255 * overallAlpha))
    local exl = ex - hc * 33 + hs * 104
    local eyl = ey - hs * 33 - hc * 104
    local exr = ex + hc * 32 + hs * 89
    local eyr = ey + hs * 32 - hc * 89
    local ecos, esin = 0, 0
    ...
    dxDrawImage(exl - 32, eyl - 32, 64, 64, sightlangStaticImage[6], hr + 19.67, 0, 0, tocolor(255, 255, 255, 255 * overallAlpha))
    exr = exr + ecos * 0.96875
    eyr = eyr + esin * 0.96875
    sightlangStaticImageUsed[6] = true
    if sightlangStaticImageToc[6] then
      processsightlangStaticImage[6]()
    end
    dxDrawImage(exr - 31, eyr - 31, 62, 62, sightlangStaticImage[6], hr + 16.56, 0, 0, tocolor(255, 255, 255, 255 * overallAlpha))
  end
  if 0.975 < p or 0 < bunnySleeping then
    sightlangStaticImageUsed[7] = true
    if sightlangStaticImageToc[7] then
      processsightlangStaticImage[7]()
    end
    dxDrawImage(ix + hx, iy + hy, 500, 372, sightlangStaticImage[7], hr, 14, 0, tocolor(255, 255, 255, 255 * overallAlpha))
  end
  sightlangStaticImageUsed[8] = true
  if sightlangStaticImageToc[8] then
    processsightlangStaticImage[8]()
  end
  dxDrawImage(ix + hx, iy + hy, 500, 372, sightlangStaticImage[8], hr, 14, 0, tocolor(255, 255, 255, 255 * overallAlpha))
  if 0 < bunnySleeping then
    sightlangStaticImageUsed[9] = true
    if sightlangStaticImageToc[9] then
      processsightlangStaticImage[9]()
    end
    dxDrawImage(ix + hx, iy + hy, 500, 372, sightlangStaticImage[9], hr, 14, 0, tocolor(255, 255, 255, 255 * overallAlpha))
    if 1000 < now - lastWord then
      ...
      dxDrawImage(ix + hx, iy + hy, 500, 372, sightlangStaticImage[10], hr, 14, 0, tocolor(255, 255, 255, 255 * overallAlpha))
    end
  end
  ...
  if nearestItemId then
    ...
    dxDrawText(bunnyNames[nearestItemId], 1, iy + 290 - 125 + 230 + 1, screenX + 1, 0, tocolor(0, 0, 0, 150 * a * targetAlpha * overallAlpha), fontScale * (0.8 + rotateEndedP * 0.2), font, "center", "top")
    if goldItems[nearestItemId] then
      dxDrawText(bunnyNames[nearestItemId], 0, iy + 290 - 125 + 230, screenX, 0, tocolor(245, 206, 70, 255 * a * targetAlpha * overallAlpha), fontScale * (0.8 + rotateEndedP * 0.2), font, "center", "top")
    else
      dxDrawText(bunnyNames[nearestItemId], 0, iy + 290 - 125 + 230, screenX, 0, tocolor(255, 255, 255, 255 * a * targetAlpha * overallAlpha), fontScale * (0.8 + rotateEndedP * 0.2), font, "center", "top")
    end
  end
  for i = 1, #snow do
    if snow[i][8] then
