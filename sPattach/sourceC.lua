...
sPattach = {
  ...
  setRotationMatrix = function(self, element, m)
    assert(isElement(element), "Expected element at argument 1, got " .. type(element))
    if not self:isAttached(element) then
      return false
    end
  ...
  setRotationQuaternion = function(self, element, q)
    assert(isElement(element), "Expected element at argument 1, got " .. type(element))
    if not self:isAttached(element) then
      return false
    end
  ...
  disableScreenCheck = function(self, element, state)
    assert(isElement(element), "Expected element at argument 1, got " .. type(element))
    if not self:isAttached(element) then
      return false
    end
  ...
  onStreamIn = function()
    sPattach:addToStream(source)
  end,
  onStreamOut = function()
    sPattach:removeFromStream(source)
  end,
  onDimensionChange = function(old, new)
    if sPattach.pedInstances[source] then
      for element in pairs(sPattach.pedInstances[source].list) do
  ...
  onElementDestroy = function()
    sPattach:detach(source)
  end,
  onPedDestroy = function()
    sPattach:detachAll(source)
  end,
  refreshRender = function(self)
    ...
    if 0 < len and not self.pedsProcessedAdded then
      addEventHandler("onClientPedsProcessed", root, self.onPedsProcessed, true, "low-999999999")
      self.pedsProcessedAdded = true
    elseif len == 0 and self.pedsProcessedAdded then
      removeEventHandler("onClientPedsProcessed", root, self.onPedsProcessed, true, "low-999999999")
  ...
  onPedsProcessed = function()
    local boneMatCache = {}
    for i = 1, #sPattach.preparedToRenderInstances do
      local data = sPattach.preparedToRenderInstances[i]
      local element, ped = data.element, data.ped
      local boneid = data.boneid
      local ox, oy, oz = data.ox, data.oy, data.oz
      if isOnScreen(ped) or data.disableScreenCheck then
      ...
      elseif not data.prevOutOfScreen then
        setPosition(element, 0, 0, 10000)
...
addEventHandler("onClientResourceStart", resourceRoot, function()
  triggerServerEvent("sPattach:requestCache", resourceRoot)
end)
addEvent("sPattach:receiveCache", true)
addEventHandler("sPattach:receiveCache", resourceRoot, function(cache)
  for _, data in pairs(cache) do
    ...
    sPattach:attach(unpack(data))
    if q then
      sPattach:setRotationQuaternion(data[1], q)
    end
  end
end)
